...
shared_ptr<Directory> mkdir(Node *pCurrent, const string& path)
{
   string sub_path = subpath(path); 

   if (sub_path.empty()) {

        // TODO: The time is hardcode for now
        shared_ptr<Directory> pDir = make_shared( Directory(path, std::string("10/10/2013")) ); 
